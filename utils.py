...
def split_train_test(X: pd.DataFrame, y: pd.Series, train_proportion: float = .75) \
    ...
    """
    # Randomly sample a training set of the given data frame:
    train_X = X.sample(n=None, frac=train_proportion, replace=False, weights=None,
                       random_state=None, axis=None, ignore_index=False)
    train_indexes = train_X.index
    train_y = y.loc[train_indexes]

    # The matching test set:
    all_data_indexes = X.index
    test_indexes = all_data_indexes.difference(train_indexes)
    test_X = X.loc[test_indexes]
    test_y = y.loc[test_indexes]
